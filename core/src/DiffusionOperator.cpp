    ...
    , d_y_max_value( y_max_value )
{
    if ( order == 2 )
    {
	build_second_order( num_x, num_y, dx, dy, dt, alpha );
    }
    else if ( order == 4 )
    {
	build_fourth_order( num_x, num_y, dx, dy, dt, alpha );
    }
}
...
/*!
 * \brief Build the second order diffusion operator.
 */
void DiffusionOperator::build_second_order( const int num_x, 
					    const int num_y,
					    const double dx,
					    const double dy,
					    const double dt,
...
/*!
 * \brief Build the fourth order diffusion operator.
 */
void DiffusionOperator::build_fourth_order( const int num_x, 
					    const int num_y,
					    const double dx,
					    const double dy,
					    const double dt,
